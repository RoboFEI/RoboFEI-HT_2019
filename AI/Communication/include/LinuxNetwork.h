...
namespace Robot
{
    ...
    class LinuxClient : public LinuxSocket
    {
    public:
        LinuxClient ( int port );
        LinuxClient (){};
        virtual ~LinuxClient();

        const LinuxClient& operator << ( const std::string& ) const;
        const LinuxClient& operator << ( const int& ) const;
        const LinuxClient& operator >> ( std::string& ) const;
