...
LinuxClient::LinuxClient(int port)
{
    if ( ! LinuxSocket::create() )
    {
        throw LinuxSocketException ( "Could not create server socket." );
...
const LinuxClient& LinuxClient::operator << ( const std::string& s ) const
{
    if ( ! LinuxSocket::send ( s ) )
    {
        throw LinuxSocketException ( "Could not write to socket." );
...
const LinuxClient& LinuxClient::operator << ( const int& i ) const
{
    std::stringstream ss;
    ss << i;
...
const LinuxClient& LinuxClient::operator >> ( std::string& s ) const
{
    if ( ! LinuxSocket::recv ( s ) )
    {
        throw LinuxSocketException ( "Could not read from socket." );
...
void LinuxClient::connect( const std::string host, int port )
{
    if ( ! LinuxSocket::connect(host, port) )
    {
        throw LinuxSocketException ( "Could not connect server socket." );
    }
}
